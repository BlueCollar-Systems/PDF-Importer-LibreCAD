...
- [x] **App parts_bootstrap rows ingestion (R5-2)** — `import_report_ingestion.dart` accepts `rows[]` + `source_pdf`; KettleTagService + Report Doctor screen shipped
...
- [ ] **KettleTag full loop (R5-1/R5-3)** — KettleTagService + Report Doctor shipped; scan→lookup reverse-highlight UI not built
...
| App | `0d634c4` (+ `17008ba` KettleTag) | pushed | flutter analyze clean; 21 targeted tests pass |
